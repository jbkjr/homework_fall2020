...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        ac_log_probs = ac_dist.log_prob(actions)

        if not self.discrete:
            ac_log_probs = ac_log_probs.sum(1)
        assert ac_log_probs.size() == advantages.size()

        loss = -(ac_log_probs * advantages).sum()
        ...
        if self.nn_baseline:
            ...
            train_log['Baseline Loss'] = ptu.from_numpy(baseline_loss)
